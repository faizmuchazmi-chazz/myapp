...
class Ion_auth_model extends CI_Model
{
	...
	public function change_password($identity, $old, $new)
	{
		...
		$this->set_error('password_not_valid');
		return FALSE;
	}
	...
	{
		$sesIdentity = $this->session->userdata('identity');
		if (empty($sesIdentity)) {
			return FALSE;
		}
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups)) {
			...
			if (isset($groups) && !empty($groups)) {
				...
				$this->db->join(
					...
					$this->tables['users_groups'] . '.' . $this->join[$this->tables['users']] . '=' . $this->tables['users'] . '.id',
					'inner'
				);
			}
		...
		// $this->db->join($this->config->item('tbl_ref_jabatan'), $this->config->item('tbl_ref_jabatan') . '.id = ' . $this->config->item('tbl_users') . '.jabatan_id', 'left');
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')
			->where($this->tables['users_groups'] . '.' . $this->join[$this->tables['users']], $id)
		...
		foreach ($group_ids as $group_id) {
			...
			if ($this->db->insert(
				...
				[
					...
					$this->join[$this->tables['users']]  => (float)$user_id
				]
			)) {
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		if (!empty($group_ids)) {
			...
			foreach ($group_ids as $group_id) {
				...
				$this->db->delete(
					$this->tables['users_groups'],
					[$this->join['groups'] => (float)$group_id, $this->join[$this->tables['users']] => (float)$user_id]
				);
		...
		// otherwise remove user from all groups
			...
			// Cast to float to support bigint data type
			if ($return = $this->db->delete($this->tables['users_groups'], [$this->join[$this->tables['users']] => (float)$user_id])) {
